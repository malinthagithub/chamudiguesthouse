...
const Revenue = () => {
  ...
  useEffect(() => {
    fetch('http://localhost:5000/api/revenue/analytics')
      ...
      .then((data) => {
        const monthlyRevenue = [], roomWiseRevenue = [], monthlyGuestCount = [], dailyRevenue = [];
        let currentGuestCount = 0;

        data.forEach((item) => {
          switch (item.type) {
            case 'monthly': monthlyRevenue.push(item); break;
            case 'room-wise': roomWiseRevenue.push(item); break;
            case 'monthly_guest_count': monthlyGuestCount.push(item); break;
            case 'daily': dailyRevenue.push(item); break;
            case 'current_guest_count': currentGuestCount = item.guest_count; break;
            default: break;
          }
        });
    ...
    // Fetch today's and this week's bookings
  ...
  return (
    <div className="revenue-analytics">
      ...
      <div className="dashboard-container" style={{ display: 'flex', height: '100vh' }}>
        <div className="sidebar" style={{ width: '250px', background: '#333', color: '#fff', padding: '20px' }}>
          <h3>Dashboard</h3>
